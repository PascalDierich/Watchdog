...
/**
 * Interactor to save a new Post in 'NewsFeed'
 */
        ...
        try {
            if (mItem == null) {
                mMainThread.post(new Runnable() {
                    @Override
                    public void run() {
                        FirebaseCrash.log("setNewsFeed: Item is null. \n" +
            ...
            final long postId = ApiConnector.getApi().get().setNewsFeed(mContext, mItem);

            mMainThread.post(new Runnable() {
                @Override
                public void run() {
                    mCallback.onSuccess(postId, true);
                }
            });
